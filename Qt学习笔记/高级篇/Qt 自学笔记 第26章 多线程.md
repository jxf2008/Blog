...
如果你看过《Qt Study Notes》基础篇的示例代码，那看到这里可能会有些意外，因为这是你第一次在《Qt 自学笔记》中看到析构函数，前面内存章节讲过，由于Qt内存机制使得我们大部分时候不用考虑内存的问题，所以析构函数也就不用写了，用默认的就行。但这里任然使用了析构函数，这主要设计了线程终止时，子线程的内存问题，因为用户可能在子线程还在加载图片的过程中就直接关闭了程序，对于这种情况，需要在程序析构之前把子线程停掉，如果子线程中有向系统申请的资源，也必须释放掉。
