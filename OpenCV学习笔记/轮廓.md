...
对于轮廓来说，一个很重要的用途就是用来对比，比如非常常见的手势识别，一个人举起手做一个“V”字手势，要在两张图片中识别出这个手势，直方图中介绍的模板匹配很难胜任这个人物，因为人每次比划手势都不一样，这其中包含了角度，光线，肤色等等因素影响，而通过比对手势的轮廓是一个更好的选择
...
```
初一看只要对比下两条轮廓的矩，就可以知道这两条轮廓是否相似，但实际应用时很少直接比较矩，因为矩不光包括了轮廓的外形，还有质心，旋转角度等信息。简单的说，在一张图片上画一个椭圆，然后把这个椭圆平移一段距离，或者把椭圆旋转一定度数，那对比矩的结果就是差距非常大，而实际情况更多的要求比较轮廓的外形，而忽略轮廓的平移量，旋转量，所放量，要做到这一点需要对轮廓的矩进行进一步的复杂计算，而opencv没有提供这些消除矩平移，选择，缩放量的算法，需要用户自己实现。
...
```
这两个函数便是由矩进一步计算出Hu矩，第一个函数的参数是C风格的数组，可以看出Hu矩有7个值，考虑到Hu矩的复杂性，对比Hu矩也是非常麻烦的事情，最后opencv提供了一个对比轮廓的函数
```c++
...
matchShapes()内部实现了对轮廓计算Hu矩，并对两条轮廓的Hu矩进行比较。接下来看下matchShapes()是如何比较轮廓的
