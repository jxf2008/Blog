...
模板元编程的一个很重要的功能就是输出一个类的类型，略有有区别于前面介绍的类型的识别，通过这项技术，可以让编译器在编译阶段做出一些选择从而可以在编译阶段实现类似if语句的功能，列如以下两个类
```c++
class People{
...
```
现在遇到了一个问题，一个工程中又很多个类，之间存在复制的继承关系，现在需要根据不同的类对象来调用对饮的函数，一种可行的方案是前面介绍过的类型识别,既
```c++
template<typename T>
...
```
其中class_type的实现可以参照之前介绍的方法，但这样做也存在问题。如果print_info()函数只需要处理你自己设计的类或者C++的基本类型，这种设计通常没有问题，但如果项目的需求变动，需要print_info()处理第三方的类，那么改动就比较麻烦了，最大的问题是需要修改比较核心的class_type类，一般来说，类似class_type这样的类会位于项目的比较底层的位置，有大量上层代码依赖该类，如果通过修改该类的特例化代码使得可以处理第三方类，可能在实现的时候需要小心谨慎，同时进行大量反复的测试来确保修改不会影响到已有的代码，另外你可能也需要将print_info()函数内巨大的if-else分支语句做出改动。

为了应对这些情况，另一个解决方案就是修改print_info()。该函数是通过判断T是否属于某一类型，然后做出相应的处理，这需要一个巨大的if-else语句来判断。但如果能够再print_info()函数类准确的输出T的类型，而不是判断T是否属于某一类型，问题就简单许多了，因为一旦知道了T的类型，就可以调用对应的函数

所以这里就有个2核心问题需要处理，其一，如何在print_info()中输出T的类型，其二，如何不使用if-else做分支判断

下面通过一个完整的解决方案来演示如何解决这两个问题
```c++
template<typename T>
struct ClassTraits {
	using data_traits = T;
...
```
这个类非常简单，最大的作用就是通过一种非入侵的方式（“非入侵”是指无需修改目标类的代码，这种方法尤其适用于需要处理第三方类，特别是那些非开源第三方类库的情况）来获得类类型。稍后将看到如何通过该结构获取。

第二步是封装已有的各种print_xxx_info()函数
```c++
templata<typename T>
void _print_info(const T* v , People){
	print_people_info(v);
}
templata<typename T>
void _print_info(const T* v , Worker){
	print_worker_info(v);
}
templata<typename T>
void _print_info(const T* v , Student){
	print_student_info(v);
}
//.
//.
//.
```
这一步是将所有的print_xxx_info()函数封装为_print_info()的重载,该函数的第二个参数仅用于让编译器决定调用哪个重载版本，因此不用写出具体实参，如果需要处理第三方的类对象，只要增加一个重载版本即可


第三步就是重新实现print_info()函数，我们可以看见代码非常简单，巨大的if-else分支不见了
```c++
template<typename T>
void print_info(const T* v){
	using _class_traits = typename ClassTraits<T>::data_traits;
	_print_info(v,_class_traits());
}
```
函数代码只有两行，第一行演示了如何通过ClassTraits类来获取目标类的类型，第二行则是调用函数的重载，而由于第一行获得了T的类型为_class_traits,所以编译器可以根据_class_traits来决定调用哪个重载

再整体看下整个处理方案，有两个很大的优势，其一是更严密的封装，如果需要print_info()函数处理更多的XX类，只要添加对应的_print_info(const T* v , XX)封装即可，这对已有代码的影响降到最小，也避免了去修改某些底层的核心代码。
